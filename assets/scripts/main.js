  ...
  var liTags = $('#quizz li'); // Pointeur vers tous les elem LI
  ...
  function makeQuizz() {
    ...
    liTags.each(function(index) { // Boucle qui rempli la liste de réponses
  ...
  function makeRecap() {
    ...
    liRecapTags.each(function(index) {
      if (!userAnswer[index]) {
        $(this).text('Oops, vous n\'avez pas répondu à cette question').addClass('list-group-item-warning');
      } else {
        $(this).text(userAnswer[index] + '\n');
        var answers = questionsList[index].split(','); // Découpe la chaine
        rightAnswer = answers[answers.indexOf('r') - 1]; // Récupère l'indice de la bonne réponse
        if (userAnswer[index] == rightAnswer) {
          counterStrike++;
          $(this).addClass('list-group-item-success');
        } else {
          $(this).addClass('list-group-item-danger');
          $(this).text('La bonne réponse était : ' + rightAnswer);
        }
      }
    });
  }
